...
type Config struct {
	...
	AccessToken struct {
		...
		AccessTokenExpiry  int    `yaml:"AccessTokenExpiry" env:"AT_ACCESS_TOKEN_EXPIRY",overwrite`
		RefreshTokenSecret string `yaml:"RefreshTokenSecret" env:"AT_REFRESH_TOKEN_SECRET",overwrite`
		RefreshTokenExpiry int    `yaml:"RefreshTokenExpiry" env:"AT_REFRESH_TOKEN_EXPIRY",overwrite`
	} `yaml:"AccessToken"`
